...
<head>
    ...
    <title>Agriculture Business - Uganda</title>
    ...
    <style>
        body {
            background: linear-gradient(135deg, #65a30d 0%, #d9f99d 100%);
            min-height: 100vh;
            margin: 0;
            padding: 0;
            display: flex;
        ...
        .glass-card {
            ...
            box-shadow: 0 8px 32px 0 rgba(101, 163, 13, 0.18);
            max-width: 410px;
            width: 100%;
        ...
        .glass-contact {
            ...
            border-radius: 1rem;
            padding: 0.8rem 1rem;
            box-shadow: 0 2px 8px rgba(101,163,13,0.04);
            margin-bottom: 1.2rem;
        }
        .glass-contact div {
        ...
        .glass-btn {
            ...
            width: 100%;
            padding: 0.7rem;
            background: linear-gradient(90deg, #65a30d 0%, #a3e635 100%);
            color: #fff;
            font-weight: 600;
            border: none;
            border-radius: 1rem;
            font-size: 1.05rem;
            box-shadow: 0 2px 8px rgba(101,163,13,0.08);
        ...
        .gallery-item {
            border-radius: 1rem;
            overflow: hidden;
            box-shadow: 0 2px 8px rgba(101,163,13,0.06);
            cursor: pointer;
            position: relative;
        }
        .gallery-item img {
        ...
        .gallery-caption {
            ...
            left: 0;
            right: 0;
            background: linear-gradient(transparent, rgba(101,163,13,0.7));
            color: #fff;
            font-size: 0.85rem;
...
<body>
    <div class="glass-card">
        ...
        <div class="relative w-full h-48 bg-cover bg-center rounded-t-2xl" style="background-image: url('{{ $profile->background_image_url }}');">
            ...
            <div class="absolute inset-0 bg-gradient-to-br from-lime-600/40 via-green-500/30 to-lime-700/40 rounded-t-2xl"></div>
            
            <!-- Profile Image Overlapping -->
            <div class="absolute -bottom-12 left-6">
                <img src="{{ $profile->full_profile_image_url ?? 'https://via.placeholder.com/90x90/65a30d/ffffff?text=' . substr($user->name ?? 'AG', 0, 2) }}" alt="Avatar" class="avatar border-4 border-white">
            </div>
            
        </div>
        <!-- Spacer and Profile Info -->
        <div class="pt-16 pb-6 px-6">
            <div class="glass-title">{{ $profile->display_name ?? $user->name ?? 'Agri Business' }}</div>
            <div class="glass-role">{{ $profile->profession ?? 'Agriculture Specialist' }}</div>
        ...
        <div class="glass-header">
            <img src="{{ $profile->full_profile_image_url ?? 'https://via.placeholder.com/90x90/65a30d/ffffff?text=' . substr($user->name ?? 'AG', 0, 2) }}" alt="Avatar" class="avatar">
        ...
        <div class="glass-bio">
            {{ $profile->bio ?? 'Empowering Uganda through sustainable agriculture and innovation.' }}
        </div>
    ...
    <script>
        function saveContact() {
            const name = "{{ $profile->display_name ?? $user->name ?? 'Agri Business' }}";
            const title = "{{ $profile->profession ?? 'Agriculture Specialist' }}";
