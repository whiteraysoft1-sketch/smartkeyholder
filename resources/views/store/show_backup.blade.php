...
<style>
    :root {
        --store-primary: {{ $profile->store_primary_color ?? '#3B82F6' }};
        --store-secondary: {{ $profile->store_secondary_color ?? '#10B981' }};
        --store-text: {{ $profile->store_text_color ?? '#1F2937' }};
        --store-bg: {{ $profile->store_background_color ?? '#FFFFFF' }};
    }
    
    /* Apply custom colors to buttons and key elements */
    .bg-blue-600, .bg-blue-500 {
        background-color: var(--store-primary) !important;
    }
    .bg-blue-700 {
        background-color: color-mix(in srgb, var(--store-primary) 90%, black) !important;
    }
    .text-blue-600 {
        color: var(--store-primary) !important;
    }
    .bg-green-500, .bg-green-600 {
        background-color: var(--store-secondary) !important;
    }
    .bg-gradient-to-r.from-blue-600 {
        background: linear-gradient(to right, var(--store-primary), color-mix(in srgb, var(--store-primary) 80%, var(--store-secondary))) !important;
    }
            ...
            <div class="flex items-center justify-between h-14 md:h-16">
                ...
                <div class="flex items-center space-x-2 md:space-x-4">
                    @if($profile->store_logo)
                        <img src="{{ $profile->store_logo_url }}" alt="{{ $profile->store_name }}" class="h-8 md:h-10 w-auto">
                    @endif
